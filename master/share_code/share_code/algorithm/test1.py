...
def create_examples(DF,detect,pro=1.0,msd_len=100):  #DF是pandas，pro可选，这里默认为1，意味着train/test数据集都生成，msd_len默认msd距离长为100,detect是目标列
	...
	start=i
	i+=100
        end=start+100 if start+100<=length else length  #训练集不得超过length
        tempDF=DF.iloc[start:end,:]
	if len(np.nonzero(tempDF.iloc[:,detect]==-1)[0])>=int(0.1*len(tempDF)):    #限制为10%的好处是，可以避免这种情况：两个序列中，有一个序列非常多-1，导致计算不准确
	    print('Not satisfied!')
	    continue
	simL=[]
	for j in range(tempDF.shape[1]):
		...
		if l<=int(0.1*len(tempDF)):    #如果-1占比不超过10%,计算
		    ...
		    print('%d col is not suitable for %d because -1 too much '%(j,detect))
...
def use_algorithm(Data,Label,pro=0.7):   #利用sklearn的神经网络实现预测，默认0.7训练集
    ...
    # 神经网络
    parameters = {  
        'hidden_layer_sizes': [(15,),(7,),(21,),(15,3),(7,3),(21,3),(7,3,3),(7,5,3)], 
	'max_iter': [20000,100000],
	'momentum': [0,0.5,1],
