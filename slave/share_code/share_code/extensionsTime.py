...
class Latencies(object):
    ...
    def _log(self, spider):
        irate = float(self.items) / self.interval  #interval 时间内处理item数
        latency = self.latency / self.items if self.items else 0  #单个时延
        proc_latency = self.proc_latency / self.items if self.items else 0  #单个响应时间
        ...
        self.latency, self.proc_latency, self.items = 0, 0, 0
	spider.logger.info('now init values again: %d %d %d'%(self.latency, self.proc_latency, self.items))
